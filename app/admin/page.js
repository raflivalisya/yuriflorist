...
export default function AdminPage() {
  ...
  const [subcategory, setSubcategory] = useState(''); // State baru untuk Sub-Kategori / Jenis Bunga
  ...
  async function handleSubmit(e) {
    ...
    if (!name || !price || !image || !subcategory) {
      alert('Mohon isi Nama, Harga, Kategori, Jenis Bunga (Sub-Kategori), dan Link Gambar!');
      return;
    }
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex justify-between items-center mb-8 bg-white p-4 sm:p-6 rounded-2xl border border-[#EFE8DE] shadow-sm">
          <div>
            ...
            <p className="text-xs sm:text-sm text-[#6E5B58]">Kelola katalog berdasarkan Folder Utama & Sub-Folder Jenis Bunga.</p>
          </div>
          <a href="/" className="text-xs font-semibold bg-[#FAF7F2] border border-[#E8DDD1] text-[#6E5B58] hover:bg-[#E8A5C2] hover:text-white px-3 sm:px-4 py-2 rounded-full transition-all">
            ← Kembali ke Web
          </a>
        </div>
        ...
        <div className="bg-white p-4 sm:p-6 rounded-2xl border border-[#EFE8DE] shadow-sm mb-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <input 
                  ...
                  placeholder="Contoh: Russian Pink Bouquet Part 1"
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {/* Folder Utama */}
              <div>
                <label className="block text-xs font-bold text-[#6E5B58] mb-1">Folder Utama (Kategori) *</label>
                <select
                  value={category}
              ...
              {/* Sub-Folder Jenis Bunga */}
              <div>
                <label className="block text-xs font-bold text-[#6E5B58] mb-1">File / Jenis Bunga (Sub-Folder) *</label>
                <input 
                  type="text" 
                  value={subcategory}
                  onChange={(e) => setSubcategory(e.target.value)}
                  placeholder="Contoh: Russian Pink Mix Dried"
                  className="w-full px-3 py-2 text-sm border border-[#E8DDD1] rounded-xl focus:outline-none focus:border-[#E8A5C2] bg-[#FAF7F2]"
                  required
                />
              </div>
            ...
            <div>
              ...
              <input 
                ...
                placeholder="Tempelkan link share foto Google Drive"
                className="w-full px-3 py-2 text-sm border border-[#E8DDD1] rounded-xl focus:outline-none focus:border-[#E8A5C2] bg-[#FAF7F2]"
                required
              />
            </div>
        ...
        <div className="bg-white p-4 sm:p-6 rounded-2xl border border-[#EFE8DE] shadow-sm">
          ...
          {loading ? (
            ...
            <div className="space-y-3">
              {products.map((item) => (
                <div key={item.id} className="flex flex-col sm:flex-row items-start sm:items-center justify-between p-3 border border-[#EFE8DE] rounded-xl bg-[#FAF7F2] gap-3">
                  <div className="flex items-center gap-3">
                    <img src={formatDriveUrl(item.image)} alt={item.name} className="w-12 h-12 rounded-lg object-cover bg-white" referrerPolicy="no-referrer" />
                    <div>
                      ...
                      <div className="flex flex-wrap items-center gap-1.5 mt-0.5">
                        ...
                        <span className="bg-[#F8E3EC] text-[#B85B84] text-[9px] font-bold px-2 py-0.5 rounded-full">
                          📁 {item.category}
                        </span>
                        <span className="bg-[#E8DDD1] text-[#6E5B58] text-[9px] font-bold px-2 py-0.5 rounded-full">
                          📄 {item.subcategory}
                        </span>
                      </div>
                    </div>
                  ...
                  <div className="flex items-center gap-2 self-end sm:self-auto">
                    <button onClick={() => handleEdit(item)} className="text-xs bg-[#E8DDD1] text-[#4A3E3D] px-3 py-1.5 rounded-lg font-semibold">Edit</button>
                    <button onClick={() => handleDelete(item.id)} className="text-xs bg-red-100 text-red-600 px-3 py-1.5 rounded-lg font-semibold">Hapus</button>
                  </div>
                </div>
              ))}
            </div>
